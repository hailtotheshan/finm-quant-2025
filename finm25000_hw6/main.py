...
def main():
    # 1. Hard-coded “first 100” tickers you generated
    tickers_sp500_100 = [
        "MMM", "AOS", "ABT", "ABBV", "ACN", "ADBE", "AMD", "AES", "AFL", "A",
        "APD", "AKAM", "ALB", "ARE", "ALGN", "ALLE", "LNT", "ALL", "GOOGL",
        "GOOG", "MO", "AMZN", "AMCR", "AEE", "AEP", "AXP", "AIG", "AMT", "AWK",
        "AMP", "AME", "AMGN", "APH", "ADI", "AON", "APA", "APO", "AAPL", "AMAT",
        "APTV", "ACGL", "ADM", "ANET", "AJG", "AIZ", "T", "ATO", "ADSK", "ADP",
        "AZO", "AVB", "AVY", "AXON", "BKR", "BALL", "BAC", "BAX", "BDX", "BRK-B",
        "BBY", "TECH", "BIIB", "BLK", "BX", "XYZ", "BK", "BA", "BKNG",
        "BSX", "BMY", "AVGO", "BR", "BRO", "BF-B", "BLDR", "BG", "BXP",
        "CHRW", "CDNS", "CZR", "CPT", "CPB", "COF", "CAH", "KMX", "CCL",
        "CARR", "CAT", "CBOE", "CBRE", "CDW", "COR", "CNC", "CNP", "CF",
        "CRL", "SCHW", "CHTR", "CVX", "SPY"
    ]

    # 2. Instantiate loader for daily 5-year bars
    loader = MarketDataLoader(interval="1d", period="5y")

    # 3. Loop and fetch via loader.get_history(); store each ticker's last_price
    price_series = {}
    for ticker in tickers_sp500_100:
        try:
            df = loader.get_history(ticker)  # returns DataFrame indexed by UTC timestamps
            price_series[ticker] = df["last_price"]
        except Exception as e:
            # if fetch fails, fill series with NaNs
            price_series[ticker] = pd.Series(dtype=float)
            print(f"⚠️  failed to load {ticker}: {e}")

    # 4. Build a wide DataFrame
    prices = pd.DataFrame(price_series)

    # 5. Reindex to full business-day UTC calendar & ffill gaps
    ...
    prices = prices.reindex(full_idx).fillna(method="ffill")

    df = prices.drop(columns=['SPY'])
    ...
    df_ew = pd.concat([pd.Series(ew_values, name='Portfolio'), prices['SPY']], axis=1)
    df_sw = pd.concat([pd.Series(sw_values, name='Portfolio'), prices['SPY']], axis=1)
    ...
    ew_hedging = implement_hedging_strategy(prices, ew_values, ew_beta_results)
    # Signal-weighted hedging (using its own beta)
    sw_hedging = implement_hedging_strategy(prices, sw_values, sw_beta_results)
    ...
    spy_prices = prices['SPY']  # series of SPY adj. closes
    ...
    print("""Further improvements in risk-adjusted returns could be achieved by 1) adding more
    technical indicators to the signal-weighted strategy and 2) implementing statistical machine
    learning models to predict future returns based on historical data and technical indicators.
    In this program, signal generation is based on two simple technical indicators: RSI and MACD.
    However, more complex indicators such as Bollinger Bands, Stochastic Oscillator, can 
    potentially improve the performance of the signal-weighted strategy. Additionally, statistical
    machine learning models such as Random Forest, Gradient Boosting, or Neural Networks can be 
    used to replace simple signal generation methods. These models can learn complex patterns in 
    in the data and achieve better predictions of future returns. By combining these improvements,
    signal-weighted portfolio may outperform the equal-weighted portfolio in terms of risk-adjusted 
    returns.
    In this program, stability and downside control are enhenced from hedging with rolling SPY beta.
    As shown in the performance metrics, both hedged equal-weighted portfolio and hedged signal 
    weighted portfolio has lower annualized volatility and maximum drawdown compared to the 
    unhedged version. The results in performance metrics highlight the importance of hedging 
    in risk management and portfolio stability.
    Tail risk varied between the two portfolios, with the hedged signal-weighted portfolio
    having higher tail risk than the hedged equal-weighted portfolio in terms of VaR and CVaR.
    This suggests that the hedged signal-weighted portfolio may be more sensitive to extreme market.
    In addition, both hedged portfolios have lower skewness and kurtosis compared to their unhedged
    versions, indicating that hedging has helped to reduce the asymmetry and fat tails of the return distributions.""")
